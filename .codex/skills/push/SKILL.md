...
2. Run local validation (`go test ./...`) before pushing.
...
6. Write/update PR body explicitly to match the repo workflow requirements:
   - Include four sections: `## 背景`、`## 本次改动`、`## 验证`、`## 关联`.
   - Ensure the current issue identifier is mentioned in `## 关联`.
   - If PR already exists, refresh body content so it reflects the total PR
     scope (all intended work on the branch), not just the newest commits,
     including newly added work, removed work, or changed approach.
   - Do not reuse stale description text from earlier iterations.
7. Validate that the PR body contains those four required sections and fix it if not.
...
# Write/edit PR body to match the workflow-required sections before validation.
# Example workflow:
# 1) draft body content with: ## 背景 / ## 本次改动 / ## 验证 / ## 关联
# 2) gh pr edit --body-file /tmp/pr_body.md
# 3) for branch updates, re-check that title/body still match current diff

tmp_pr_body=$(mktemp)
gh pr view --json body -q .body > "$tmp_pr_body"
rg -q "^## 背景" "$tmp_pr_body"   && rg -q "^## 本次改动" "$tmp_pr_body"   && rg -q "^## 验证" "$tmp_pr_body"   && rg -q "^## 关联" "$tmp_pr_body"
rm -f "$tmp_pr_body"
