...
- Primary runtime log: the path passed via `--log-file`
  - In this repo, smoke and integration runs commonly write to files under `logs/`.
  - It includes orchestrator, agent runner, and Codex app-server lifecycle logs.
- Related runtime logs: `logs/*.log` (or the operator-provided log file path)
  - Check adjacent log files when the relevant run is older or split across multiple smoke runs.

## Correlation Keys

- `issue_identifier`: human ticket key (example: `IIWATE-37`)
- `issue_id`: Linear UUID (stable internal ID)
- `session_id`: Codex thread-turn pair (`<thread_id>-<turn_id>`)

Current Go implementation writes structured issue/session fields via `internal/logging/logging.go` (`WithIssue`, `WithSession`). Use them as your join keys during debugging.
...
rg -n "issue_identifier=IIWATE-37" logs

# 2) If needed, narrow by Linear UUID
rg -n "issue_id=<linear-uuid>" logs

# 3) Pull session IDs seen for that ticket
rg -o "session_id=[^ ;]+" logs | sort -u

# 4) Trace one session end-to-end
rg -n "session_id=<thread>-<turn>" logs

# 5) Focus on stuck/retry signals
rg -n "Issue stalled|scheduling retry|turn_timeout|turn_failed|Codex session failed|Codex session ended with error" logs
```
...
In Symphony-Go, Codex session diagnostics are emitted into the log file you pass
via `--log-file` (commonly files under `logs/`) and keyed by `session_id`.
...
- Check rotated logs (`logs`) before concluding data is missing.
- If required context fields are missing in new log statements, align with the
  current Go logging helpers in `internal/logging/logging.go`.
