...
- `仅文档`：当前以 runbook / checklist / real integration report 为主要证据
...
| `attempt` 模板变量与更多 typed error 细项 | `internal/workflow`、`internal/config` | 部分覆盖 | 上述 workflow/config 测试 | `REQUIREMENTS.md`、`FLOW.md` | `attempt` 有实现链路，但缺显式单测锚点 |
...
| transport failure / 更广义 malformed payload | `internal/tracker` | 部分覆盖 | 现有 tracker 测试 | `docs/release-checklist.md` | 主要错误面已覆盖，仍可补更细的 transport/malformed 负例 |
...
| slot exhaustion requeue、max backoff 封顶 | `internal/orchestrator` | 部分覆盖 | 现有 orchestrator 测试 | `REQUIREMENTS.md` | 仍建议补显式单测锚点 |
...
| turn timeout / 非 JSON stderr / rate-limit 提取 | `internal/agent` | 部分覆盖 | 现有 agent 测试 | `docs/real-integration-report-2026-03-07.md` | 仍建议补更细单测 |
...
| rate-limit 聚合展示 | `internal/orchestrator`、`internal/server` | 部分覆盖 | 现有状态快照测试 | `docs/release-checklist.md` | 缺直接单测锚点 |
...
| timeout / unavailable / rate-limit 展示 | `internal/server` | 部分覆盖 | 现有 server 测试 | `docs/real-integration-report-2026-03-07.md` | 仍可补更细负例 |
...
| GraphQL errors / invalid arguments / unsupported tool | `internal/agent` | 已覆盖 | `internal/agent/runner_test.go`：`TestRunnerLinearGraphQLToolGraphQLErrors`、`TestRunnerLinearGraphQLToolInvalidArguments`、`TestRunnerAutoApprovesAndRejectsUnsupportedToolCalls` | `docs/real-integration-report-2026-03-07.md` |  |
| missing auth / transport failure / 广告负例 | `internal/agent` | 后续补强 | 现有 agent 测试 | `docs/release-checklist.md` | 非首版阻塞项 |

## Real Integration Profile (`§17.8`)

| SPEC 条目 | 实现包 | 现状 | 测试锚点 | 文档锚点 | 备注 |
|---|---|---|---|---|---|
| 真实凭据 smoke：`--dry-run`、候选 issue、HTTP/SSE/refresh、真实 issue 调度、dynamic tool | 全链路 | 仅文档 | 无自动化 scaffolding | `docs/real-integration-report-2026-03-07.md` | 当前以手工验证 + 报告为主 |
...
- `HTTP Server` 与 `linear_graphql` 已 shipped，但仍有少量负例未形成单测锚点：
  - `linear_graphql` 的 missing auth / transport failure
  - dynamicTools 广告负例（`tracker.kind != linear` 或 `api_key` 为空）
- `Core Conformance` 中仍建议后续补显式单测：
  - orchestrator 的 slot exhaustion requeue
  - `max_retry_backoff_ms` 封顶
  - agent 的 turn timeout / rate-limit 提取 / 非 JSON stderr
