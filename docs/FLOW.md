...
- `automation/local/session-state.json`（历史默认）
- `automation/local/../local/session-state.json` 在当前实现中已统一收敛为 `local/session-state.json`
...
- `[实现现状]` `runtime.session_persistence` 仍属于 `restart-required`；当前实现不会在 reload 时热切换 state store。
- `[实现现状]` `runtime.notifications` 支持热更新，但切换后仅影响后续事件；reload 会清理已删除通道对应的瞬时健康状态。
- `[实现现状]` `local/session-state.json` 只承载本地 durable runtime state，不保存 secret，也不会参与 watcher reload；相对路径始终相对于 automation root 解析。
- `[实现现状]` 若 `local/session-state.json` 的兼容签名或版本不匹配，启动会明确失败并要求先删除状态文件。
...
- `session_persistence.kind`
- `session_persistence.file.path`
- `session_persistence.file.flush_interval_ms`
- `session_persistence.file.fsync_on_critical`
- `notifications.channels[].id`
- `notifications.channels[].display_name`
- `notifications.channels[].kind`
...
- `session_persistence.kind = file`
- `session_persistence.file.path = ./local/session-state.json`
- `session_persistence.file.flush_interval_ms = 1000`
- `session_persistence.file.fsync_on_critical = true`
- `notifications.defaults.timeout_ms = 5000`
- `notifications.defaults.retry_count = 2`
- `notifications.defaults.retry_delay_ms = 1000`
- `notifications.defaults.queue_size = 128`
- `notifications.defaults.critical_queue_size = 32`
...
- `[实现扩展]` `CompleteIssue` 不在 `Client` 上，而是在独立 `IssueCompleter` 接口上。
...
2. 优先按 `type=completed` 选择工作流终态
3. 如果没找到，再退回到名称为 `Done` 的状态
...
- `[实现决策]` “优先 `type=completed`，再回退到名称 `Done`” 是当前实现自己的决策，而不是 SPEC 强制要求。
...
- `RequestRefresh` 的 HTTP 返回现在显式区分 `accepted` 与 `coalesced`，不再把所有请求都表述成“已排队”。
...
- 不在 `Running` 中
- 不在 `RetryAttempts` / `AwaitingMerge` / `AwaitingIntervention` 中
...
1. 创建 worker context
2. 停掉已有 retry timer
3. 从 `RetryAttempts` 删除旧记录
4. 把上一次 retry 条目里的 `Attempt/StallCount` 投影到新的 `RunningEntry`
5. 在 `Running` 中创建 `RunningEntry`
6. 刷新并发布快照
...
2. PR merged 且 `orchestrator.auto_close_on_pr=true`：尝试 `CompleteIssue()`
...
- `Retrying[]`
  ...
  - `Error`
- `Health.Alerts[]`
  - 真正有 set/clear 生命周期的系统级告警
- `Observations.Derived[]`
  - 由 `RetryAttempts` / `AwaitingMerge` 派生出的观察项
- `Observations.ProtectedResults[]`
  - protected mode 下已完成但未正式入账的运行结果
- `CodexTotals`
- `RateLimits`
...
- 系统级告警与派生观察项都会按 `code -> issue_identifier -> message` 排序，保证状态面稳定可读
...
| `POST` | `/api/v1/refresh` | best-effort 触发一次立即轮询（success=202，protected=409） |
| `GET` | `/api/v1/events` | SSE 快照流 |
| `GET` | `/api/v1/{identifier}` | 按 issue identifier 查询单个 issue 状态（unknown/malformed=404 JSON error envelope） |
...
- `service.mode`
- `service.protection_reason`
- `service.protected_at`
...
- `running[]`
- `recovery.recovering[]`
- `recovery.awaiting_merge[]`
- `recovery.awaiting_intervention[]`
- `recovery.retrying[]`
- `health.alerts[]`
- `health.notifications[]`
- `health.persistence`
- `observations.derived[]`
- `observations.protected_results[]`
- `codex_totals`
- `rate_limits`
...
- `status` 当前可能是 `running` / `recovering` / `awaiting_merge` / `awaiting_intervention` / `retrying` / `protected_result`
...
- `[实现决策]` 若 `{identifier}` 路径为空或包含额外 `/`，当前返回 `404` + `{"error":{"code":"invalid_issue_identifier","message":"..."}}`

`POST /api/v1/refresh` 在成功时返回：
...
- 若服务已进入 protected mode，当前返回 `409`，并使用统一错误 envelope：
  - `{"error":{"code":"service_protected_mode","message":"..."}}`
...
- `mode`：当前服务模式，取值 `normal` / `protected`
- `protection_reason`：稳定返回字段；进入 protected mode 时为原因字符串，normal 模式固定为空字符串
- `protected_at`：进入 protected mode 的 UTC 时间；normal 模式为空
- `restart_required`：当前模式是否要求 operator 修复后重启
