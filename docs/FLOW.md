...
- `[实现现状]` `local/session-state.json` 只承载本地 runtime state，不保存 secret，也不会参与 watcher reload；相对路径始终相对于 automation root 解析。
- `[实现现状]` 若 `local/session-state.json` 与当前运行实例 identity 或版本不兼容，启动会明确失败并要求先删除旧文件。
